...
/**
 * This file is modified by RPG Maker MZ's original `main.js`.
 * After splitting the scripts, one can replace the `main.js` in the MZ project
 * by this one to correctly run the splitted scripts.
 */
...
class Main {
    ...
    loadMainScripts() {
            ...
            if (fs.lstatSync(url).isDirectory()) {
                fs.readdirSync(url).forEach(file => {
                    if (path.extname(file) == ".js") {
                        targetUrls.push(path.join(url, file).replaceAll("\\", "/"));
                    }
                });
            }
            else {
        ...
        this.numScripts = targetUrls.length;
